@extends('layouts.vertical', ['subtitle' => 'Agenda'])

@section('content')

@include('layouts.partials.page-title', ['title' => 'Agenda', 'subtitle' => 'Agenda'])
<style>
  #calendar {
    max-width: 1000px;
    margin: 0 auto;
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const eventosCalendario = JSON.parse(`{!! json_encode($event->map(function($evento) {
        return [
            'id' => $evento->id,
            ...
            <div class="modal-body">
              ...
              <div class="mb-3">
                ...
                <select name="tipo_atencion" class="form-control" required>
                  ...
                  <option name="tipo_atencion" value="{{$tipos->id}}">{{$tipos->nombre}}</option>
                  @endforeach
                </select>
              </div>
              <div class="mb-3">
